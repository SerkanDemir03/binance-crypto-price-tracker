...
exports.fetchAndSavePrices = asyncHandler(async (req, res) => {
  try {
    ...
    try {
      // Custom symbols varsa sadece onları çek
      if (customSymbols && Array.isArray(customSymbols) && customSymbols.length > 0) {
        logger.info(`🔄 ${customSymbols.length} custom coin için fiyatlar çekiliyor...`);
        prices = await coingeckoService.getPricesBySymbols(customSymbols);
        usedProvider = 'coingecko'; // Custom symbols için CoinGecko kullan
      } else {
        logger.info(`🔄 ${provider === 'coingecko' ? 'CoinGecko' : 'Binance'} API'den fiyatlar çekiliyor...`);
      ...
      if (prices.length > 0) {
        logger.info(`✅ ${prices.length} kripto para fiyatı ${provider === 'coingecko' ? 'CoinGecko' : 'Binance'} API'den çekildi`);
      ...
      logger.warn(`⚠️ ${provider === 'coingecko' ? 'CoinGecko' : 'Binance'} API hatası: ${error.message}`);
